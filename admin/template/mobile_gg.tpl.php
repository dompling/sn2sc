    ...
    <form method="post" action="?">
        ...
        <div id="<?= MPS_SOFTNAME ?>">
            <table border="0" cellspacing="0" cellpadding="0" class="vbm">
                <tr class="firstr">
                    <td width="50"><input name="checkall" type="checkbox" class="checkbox" id="checkall"
                ...
                <?php foreach ($row AS $row) { ?>
                    <tr align="center" bgcolor="white">
                        <td><input type='checkbox' name='delids[]' value='<?= $row[id] ?>' class="checkbox"
                                   id="<?= $row[id] ?>"></td>
                        <td><a href="?type=gg&typeid=<?= $row['typeid'] ?>"><?php echo $target[$row['typeid']]; ?></a>
                        </td>
                        <td><a href='javascript:blocknone("pm_<?= $row[id] ?>");'><?= $row[image] ?></a></td>
                        <td><?= $row[words] ?></td>
                        <td><em><?= GetTime($row[pubdate]) ?></em></td>
                        <td><input name="displayorder[<?= $row[id] ?>]" value="<?= $row[displayorder] ?>" class="txt"/>
                        </td>
                        <td>
...
<?php } ?>
    ...
    <form method="post" id="form1" name="form1" action="?" enctype="multipart/form-data"
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="vbm">
                ...
                <tr bgcolor="#FFFFFF">
                    ...
                    <td><input type="file" name="mymps_focus" size="45" id="litpic"><br/><br/>
                        ...
                        手机版首页幻灯片尺寸：<?= $mymps_mymps[cfg_hdp_limit][width] ?>
                        * <?= $mymps_mymps[cfg_hdp_limit][height] ?><br/>
                        手机版新闻首页幻灯片尺寸：<?= $mymps_mymps[cfg_hdp_limit][width] ?>
                        * <?= $mymps_mymps[cfg_hdp_limit][height] ?><br/>
                    </td>
                </tr>
                </tbody>
        ...
        <center><input class="mymps large" type="submit" value="上 传" name="focus_submit"> <input
                    onClick="history.back(-1);" class="gray large" type="button" value="返 回"></center>
    </form>
