            ...
            <div class="panel-tab">
                ...
                <ul style="float:right; margin-right:10px">
                    <form action="?" method="get">
                        ...
                        <input name="title" type="input" value="<?php echo $title; ?>" class="text"
                               style="width:120px;"/>
                        <select name="catid">
    ...
    <form action="?part=list" method="post">
        ...
        <div id="<?= MPS_SOFTNAME ?>">
            <table border="0" cellspacing="0" cellpadding="0" class="vbm">
                <tr class="firstr">
                    <td width="60"><input class="checkbox" name="checkall" type="checkbox" id="checkall"
                ...
                <?php foreach ($news AS $row) { ?>
                    <tr bgcolor="white">
                        <td><input type='checkbox' name='delids[]' value="<?= $row['id'] ?>" class='checkbox'
                                   id="<?= $row['id'] ?>"></td>
                        <td><?= $row['id'] ?></td>
                        <td align="left" width="120"><a href="../news.php?id=<?= $row[id] ?>" target="_blank"
                                                        title="<?= $row['title'] ?>"><?= substring($row['title'], 0, 15) ?></a>
                        </td>
                        <td><?= $row['author'] ?></td>
                        <td><?= $row['source'] ?></td>
