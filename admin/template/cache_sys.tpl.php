    ...
    <form action="?part=cache_sysupdate" method="post">
        ...
        <div id="<?= MPS_SOFTNAME ?>">
            <table border="0" cellspacing="0" cellpadding="0" class="vbm">
                ...
                <tr>
                    <td class="altbg1" valign="center" style="text-align:center;width:15%"><b><input class="checkbox"
                                                                                                     name="chkall"
                                                                                                     onclick="CheckAll(this.form)"
                                                                                                     checked="checked"
                                                                                                     type="checkbox"
                                                                                                     id="chkalltables"/><label
                                    for="chkalltables"> 全选</label></b></td>
                    <td class="altbg2">
                        <?php foreach ($cachearray as $k => $v) { ?>
                            <label for="<?= $k ?>"><input checked="checked" name="updatecache[]" value="<?= $k ?>"
                                                          type="checkbox" class="checkbox" id="<?= $k ?>"><?= $v ?>
                            </label><br/>
                        <?php } ?>
                    </td>
                </tr>
