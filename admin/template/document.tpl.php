        ...
        <div class="mpstopic-category">
            <div class="panel-tab">
                <ul class="clearfix tab-list">
                    <li><a href="document.php?do=document" <?php if ($do == 'document'){ ?>class="current"<?php } ?>>已发布文档</a>
                    </li>
                ...
                <?php foreach ($docu AS $row) { ?>
                    <tr bgcolor="white">
                        <td><input type='checkbox' name='delids[]' value='<?= $row[id] ?>' class='checkbox'
                                   id="<?= $row[id] ?>"></td>
                        <td><?= $row[id] ?></td>
                        <td align="left" width="120"><a
                                    href="../store.php?user=<?= $row[userid] ?>&part=document&id=<?= $row[id] ?>"
                                    target="_blank" title="<?= $row[title] ?>"><?= substring($row[title], 0, 15) ?></a>
                        </td>
                        <td><?= $row[userid] ?></td>
                        <td><?= $row[author] ?></td>
                        <td><?= $row[source] ?></td>
