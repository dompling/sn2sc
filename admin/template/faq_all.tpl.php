    ...
    <form name='form1' method='post' action='?part=delall' onSubmit='return checkSubmit();'>
        ...
        <div id="<?= MPS_SOFTNAME ?>">
            <table border="0" cellspacing="0" cellpadding="0" class="vbm">
                <tr class="firstr">
                    <td width="60"><input name="checkall" class="checkbox" type="checkbox" id="checkall"
                ...
                <? foreach ($faq AS $faq) { ?>
                    <tr bgcolor="white">
                        <td><input type='checkbox' name='id[]' value='<?= $faq[id] ?>' id="<?= $faq[id] ?>"
                                   class='checkbox'></td>
                        <td><label><?= $faq[id] ?></label></td>
                        <td align="left"><a href="../about.php?part=faq&id=<?= $faq[id] ?>"
                                            target="_blank"><?= $faq[title] ?></a></td>
                        <td align="left"><a href="?typeid=<?= $faq[typeid] ?>"><?= $faq[typename] ?></a></td>
                        <td align="center"><a href="faq.php?part=edit&id=<?= $faq[id] ?>">编辑</a> / <a
                                    href="faq.php?part=delete&id=<?= $faq[id] ?>"
        ...
        <center><input type="submit" onClick="if(!confirm('确定要操作吗？\n\n此操作不可以恢复！'))return false;" value="提 交"
                       class="mymps large"/></center>
    </form>
