...
?>
            ...
            <tr bgcolor="#ffffff">
                <td id="menu_tip">
                    <form action="payrecord.php" method="get">
                        ...
                        时间从<input style="width:80px;" id="datepicker1" name="starttime" type="text" class="text"
                                  value="<?php echo GetTime($rstarttime, 'Y-m-d'); ?>"> - <input id="datepicker2"
                                                                                                 style="width:80px;"
                                                                                                 name="endtime"
                                                                                                 type="text"
                                                                                                 class="text"
                                                                                                 value="<?php echo GetTime($rendtime, 'Y-m-d'); ?>">
    ...
    <form action="?part=list" method="post">
        ...
        <div id="<?= MPS_SOFTNAME ?>">
            <table border="0" cellspacing="0" cellpadding="0" class="vbm">
                ...
                foreach ($list as $list) {
                    ?>
                    <tr bgcolor="white">
                        <td><input type='checkbox' name='delids[]' value='<?= $list[id] ?>' class='checkbox'
                                   id="<?= $list[id] ?>"></td>
                        <td><?= $list[orderid] ?></td>
                        <td><a href="javascript:void(0);"
                               onclick="setbg('<?= MPS_SOFTNAME ?>会员中心',400,110,'../box.php?part=member&userid=<?= $list[userid] ?>&admindir=<?= $admindir ?>')"><?= $list[userid] ?></a>
                        </td>
        ...
        <center><input type="submit" value="提 交" class="mymps large" name="<?= CURSCRIPT ?>_submit"/>
            <div class="clear"></div>
        </center>
    </form>
