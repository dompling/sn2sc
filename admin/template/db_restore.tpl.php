<?php include mymps_tpl('inc_head'); ?>
    ...
    <form name="cpform" method="post" action="?part=restore&action=dodelete">
        <div id="<?= MPS_SOFTNAME ?>">
            <table border="0" cellspacing="0" cellpadding="0" class="vbm">
                <tr class="firstr">
                    <td width="50"><input type="checkbox" name="chkall" id="chkall" class="checkbox"
                                          onclick="CheckAll(this.form)"/><label for="chkall">删?</label></td>
                    <td>文件名</td>
                    <td>版本</td>
                    <td>时间</td>
                    <td>类型</td>
                ...
                foreach ($exportlog as $key => $val) {
                    ...
                    <tr bgcolor="#ffffff">
                        ...
                        <td><a href="javascript:;" onclick="javascript:blocknone('exportlog_<?= $key ?>')"><img
                                        id="menuimg_1" src="template/images/menu_add.gif"
                                        align="absmiddle"/> <?= $key ?></a></td>
                        <td><?= $info['version'] ?></td>
                        <td><?= $info['dateline'] ?></td>
                    ...
                    <?php foreach ($val as $v) {
                        ...
                        <tr bgcolor="#ffffff">
                            <td>&nbsp;</td>
                            <td><i style="color:#666"><?= substr(strrchr($v['filename'], "/"), 1) ?></i></td>
                            <td><i style="color:#666"><?= $v['version'] ?></i></td>
                            <td><i style="color:#666"><?= $v['dateline'] ?></i></td>
                            <td><i style="color:#666"><?= $backuptype[$v['type']] ?></i></td>
                            <td><i style="color:#666"><?= $v['size'] ?></i></td>
                            <td><i style="color:#666"><?= $v['volume'] ?>#</i></td>
                            <td>&nbsp;</td>
                        </tr>
                        <?
                    }
