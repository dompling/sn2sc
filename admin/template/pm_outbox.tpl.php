        ...
        <div class="mpstopic-category">
            <div class="panel-tab">
                <ul class="clearfix tab-list">
                    <li><a href="pm.php?part=send" <?php if ($part == 'send'){ ?>class="current"<?php } ?>>群发短消息</a>
                    </li>
                    <li><a href="pm.php?part=outbox"
                           <?php if ($part == 'outbox'){ ?>class="current"<?php } ?>>已发送短消息</a></li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <form action="?action=pm" method="post">
        ...
        <div id="<?= MPS_SOFTNAME ?>">
            <table border="0" cellspacing="0" cellpadding="0" class="vbm">
                <tr class="firstr">
                    <td width="50"><input name="checkall" type="checkbox" id="checkall" onClick="CheckAll(this.form)"
                                          class="checkbox"/> 删?
                    </td>
                    <td>标题</td>
                    <td>发送者</td>
                ...
                <?php foreach ($pm AS $row) { ?>
                    <tr bgcolor="#ffffff">
                        ...
                        <td><?= $row[fromuser] ?></td>
                        <td><a href="
javascript:setbg('<?= MPS_SOFTNAME ?>会员中心',400,110,'../box.php?part=member&userid=<?= $row[touser] ?>&admindir=<?= $admindir ?>')"><?= $row[touser] ?></a>
                        </td>
                        <td>
                            <?php
                    ...
                    <tr style="background-color:white; display:none" id="pm_<?= $row[id] ?>">
                        ...
                        <td colspan="5">
                            ...
                            <div class="lit_manage"><a href="?part=send&id=<?= $row[id] ?>">转发</a> -
                                <a href="?part=del&id=<?= $row[id] ?>&url=<?= urlencode(GetUrl()) ?>">删除</a></div>
                        </td>
                    </tr>
                <? } ?>
            </table>
