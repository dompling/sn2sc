<?php include mymps_tpl('inc_head'); ?>
        ...
        <div class="mpstopic-category">
            <div class="panel-tab">
                <ul class="clearfix tab-list">
                    <li><a href="?part=optimize" <?php if ($part == 'optimize'){ ?>class="current"<?php } ?>>数据库优化</a>
                    </li>
    ...
    <form action="?part=repair" method="post">
        ...
        <div id="<?= MPS_SOFTNAME ?>">
            <table border="0" cellspacing="0" cellpadding="0" class="vbm">
                <tr class="firstr">
                    <td width="70"><label for="chkall"><input name="chkall" id="chkall" class="checkbox"
                                                              onclick="AllCheck('prefix', this.form)" checked="checked"
                                                              type="checkbox"/>修复?</label></td>
                    <td>数据表</td>
                    <td>类型</td>
                    <td>记录数</td>
                    <td>数据</td>
                ...
                foreach ($tablearray as $tp) {
                    ...
                    while ($table = $db->fetchRow($query)) {
                        if ($table[$tabletype] == 'MyISAM') {
                            ...
                            ?>
                            <tr bgcolor="#ffffff">
                                <td><input class="checkbox" type="checkbox" name="optimizetables[]"
                                           value="<?= $table[Name] ?>" <?= $checked ?>></td>
                                <td><?= $table[Name] ?></td>
