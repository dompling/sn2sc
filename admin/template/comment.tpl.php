<?= mymps_admin_tpl_global_head(); ?>
    ...
    <div class="ccc2">
        <ul>
            <form action="?" method="get">
                ...
                <label for="c0"><input name="comment_level" class="radio" type="radio"
                                       value="0" <?php if ($_GET[comment_level] == 0) {
                        echo "checked";
                    } ?> id=c0>待审 </label>
                <label for="c1"><input name="comment_level" class="radio" type="radio"
                                       value="1" <?php if ($_GET[comment_level] == 1) {
                        echo "checked";
                    } ?> id=c1>正常 </label>
    ...
    <form name='form1' method='post' action='?part=<?= $part ?>' onSubmit='return checkSubmit();'>
            ...
            <table border="0" cellspacing="0" cellpadding="0" class="vbm">
                ...
                <?php foreach ($comment AS $v) { ?>
                    <tr align="center" bgcolor="white">
                        <td><input type='checkbox' name='id[]' value='<?= $v[id] ?>' class='checkbox'
                                   id="<?= $v[id] ?>"></td>
                        <td><?= $v[id] ?></td>
                        <td><?= $v[userid] ?></td>
                        <td><?= $v[title] ?>&nbsp;</td>
                        ...
                        <td><a href="?part=<?= $part ?>&action=del&id=<?= $v[id] ?>"
                               onClick="return confirm('您确定要删除吗，如不确定请点取消')">删除</a></td>
                    </tr>
                <?php } ?>
                </tbody>
                <tr bgcolor="#ffffff" height="28">
                    <td align="center" style="border-right:1px #fff solid;"><input name="checkall" type="checkbox"
                                                                                   id="checkall"
        ...
        <center>
            <input type="submit" onClick="if(!confirm('确定要操作吗？'))return false;" value="提 交" class="mymps large"/>
        </center>
    </form>
