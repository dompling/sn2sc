...
<?php if ($mymps_global['cfg_if_corp'] != 1) { ?>
    <div class="ccc2">
        <ul>
            <img src="../images/warn.gif" align="absmiddle"> 提示：当前你的<font
                    color="red">商家功能已经关闭，团购插件，优惠券插件，商品插件已自动被禁用</font>，您可以在<a href="config.php">系统配置 -> 会员相关设置</a>里开启商家功能
        </ul>
    </div>
    <div class="clear"></div>
<?php } ?>
    <form action="?part=list" method="post">
        ...
        <div id="<?= MPS_SOFTNAME ?>">
            <table border="0" cellspacing="0" cellpadding="0" class="vbm">
                ...
                <?php foreach ($plugin as $list) { ?>
                    <tr bgcolor="white">
                        <td><i><?= $list[id] ?>.</i></td>
                        <td><?php if ($list[disable] == 1) { ?><font color="red">&times;</font><?php } else { ?><font
                                    color="green">√</font><? } ?></td>
                        <td><b><?= $list[name] ?></b></td>
                        <td><?= $list[flag] ?></td>
                        ...
                        <td align="left"><?php if ($list[iscore] != 1) { ?><a
                                href="plugin.php?op=edit&id=<?= $list[id] ?>">配置</a> | <a
                                        href="../<?= $list[flag] ?>.php"
                                        target="_blank">预览首页</a> | <?php if ($list[disable] == 0) { ?><a
                                    href="?op=disable&id=<?= $list[id] ?>"
                                    style="color:red; text-decoration:underline;">禁用&raquo;</a><? } else { ?><a
                                    href="?op=able&id=<?= $list[id] ?>" style="color:green; text-decoration:underline;">
                                        启用&raquo;</a><? } ?><?php } else { ?>N/A<?php } ?></td>
                    </tr>
                <?php } ?>
        ...
        <center><input type="submit" value="更新插件缓存" class="mymps large" name="<?php echo CURSCRIPT; ?>_submit"/>
        </center>
    </form>
