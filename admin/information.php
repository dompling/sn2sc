    ...
    default:
        if ($action == 'delall') {
            ...
            foreach ($_FILES as $file) {
                ...
                $result = ajax_upload_img($file_data, $file['name'], $filePath, 0, $mymps_mymps['cfg_coupon_limit']['width'], $mymps_mymps['cfg_coupon_limit']['height']);
                if (!$result['status']) $errorFile .= $file['name'];
                $response[] = $result;
            }
            ...
            switch ($do) {
                case 'post':
                    ...
                    $is_replace = intval($is_replace) > 0 ? 1 : 0;
                    ...
                    $sql = 'UPDATE `' . $db_mymps . 'information` SET ' . $manage_pwd . ' ' . $refreshtime . ' ' . $userid . ' 
                    title = \'' . $title . '\',
                    status = \'' . $status . '\',
                    content = \'' . $content . '\',
                    gid=\'' . $r['gid'] . '\',
                    catname=\'' . $r['catname'] . '\',
                    dir_typename=\'' . $r['dir_typename'] . '\',
                    catid = \'' . $catid . '\', 
                    areaid = \'' . $areaid . '\', 
                    ...
                    r_email = \'' . $r_email . '\' , 
                    r_tel = \'' . $r_tel . '\' , 
                    r_contact_who = \'' . $r_contact_who . '\' , 
                    upgrade_type = \'' . $upgrade_type . '\',upgrade_type_list = \'' . $upgrade_type_list . '\' ,upgrade_type_index = \'' . $upgrade_type_index . '\' , upgrade_time = \'' . $upgrade_time . '\',upgrade_time_list = \'' . $upgrade_time_list . '\', upgrade_time_index = \'' . $upgrade_time_index . '\', ifred = \'' . $ifred . '\', ifbold = \'' . $ifbold . '\',mappoint = \'' . $mappoint . '\' WHERE id = \'' . $id . '\'';
                    $db->query($sql);
                ...
                default:
                    ...
                    $post['upload_old_img'] = get_info_image($cat['if_upimg'], $id);
                    ...
                    $post['part'] = 'edit';
                    $post['url'] = $mymps_global['SiteUrl'] . '/admin/information.php';
        ...
        } else if ($action == 'id_search') {
            global $mymps_global;
            if (trim($keyword) !== '' && intval($keyword) !== 0) {
                $status = ['code' => 1, 'msg' => '', 'data' => []];
                $keyword = htmlspecialchars(intval($keyword));
                $status['data'] = $db->getRow('SELECT title,tel,qq,email,contact_who,catname,img_path FROM `' . $db_mymps . 'information` WHERE id=\'' . $keyword . '\'');

                if (empty($status['data'])) {
                    $status['msg'] = '未找到ID为<strong style="color: red">' . $keyword . '</strong>的相关信息';
                } else {
                    $status['data']['img_path'] = $status['data']['img_path'] ? $mymps_global['SiteUrl'] . $status['data']['img_path'] : '';
                }
                echo json_encode($status);
                return;
            }
            $post['url'] = $mymps_global['SiteUrl'] . '/admin/information.php';
            include mymps_tpl('m_search');
