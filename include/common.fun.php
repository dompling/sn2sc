...
function get_upload_image_view($if_upimg = 1, $infoid = '', $number = '')
{
    ...
    if ($if_upimg == 1) {
        ...
        $mymps .= '});</script>';
        $mymps .= '<div style="diplay:none;"><input type="hidden" name="" value=""></div>';
        for ($i = 0; $i < $cfg_upimg_number; $i++) {
            $mymps .= '<div class="onea_dd">
						...
						<div class="a_ddarea">
						<input type="file" name="mymps_img_' . $i . '" id="mymps_img_' . $i . '" class="comment-pic-upd" data="0" multiple="multiple"/>
						<img src="' . $mymps_global['SiteUrl'] . '/template/default/images/post/addimg.gif" alt="上传照片" title="上传图片">
						</div>
						<div class="save_btn"><button class="save" type="button" >保存</button></div>
					</div>';
        }
    }
    return $mymps;
...
//        $cfg_upimg_number = $number ? $number : ($mymps_global['cfg_upimg_number'] ? $mymps_global['cfg_upimg_number'] : 4);
        ...
        $view = $db->getAll("SELECT id,image_id,prepath,path FROM `{$db_mymps}info_img` WHERE infoid = '$infoid' order by image_id ");
        foreach ($view as $k => $v) {
            $imagei[$v['id']]['id'] = $v['id'];
