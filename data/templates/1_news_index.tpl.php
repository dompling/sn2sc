...
<div class="clear"></div>
<div class="newinfo">
<div class="hd"><? if($mymps_global['cfg_if_corp'] == 1) { ?>最新机构动态<?php } else { ?>最新发布信息<?php } ?></div>
<div class="bd">
<div id="indextop">
<div id="indextop1">
                            <? if($mymps_global['cfg_if_corp'] == 1) { ?>
                            <?php $latest_info = get_member_docu(10); ?><?php if(is_array($latest_info)){foreach($latest_info as $mymps) { ?><div class="li"><span class="tm">[<? echo GetTime($mymps['pubtime'],'y-m-d'); ?>]</span><span class="tt"><a href="<?=$mymps['uri']?>" title="<?=$mymps['title']?>" target="_blank" ><?=$mymps['title']?></a></span></div>
<?php }} ?>
                            <?php } else { ?>
                            <?php $latest_info = mymps_get_infos(10); ?><?php if(is_array($latest_info)){foreach($latest_info as $mymps) { ?><div class="li"><span class="tm">[<? echo GetTime($mymps['begintime'],'y-m-d'); ?>]</span><span class="tt"><a href="<?=$mymps['uri']?>" title="<?=$mymps['title']?>" target="_blank" ><?=$mymps['title']?></a></span></div>
<?php }} ?>
                            <?php } ?>
</div>
