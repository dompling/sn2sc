...
<?php include mymps_tpl('header'); ?>
    ...
    <meta name="description" content="<?=$mymps_global['SiteName']?>"/>
    <title><?=$mymps_global['SiteName']?></title>
        ...
        <div id="slide" style="display:none;">
            <div id="content">
                <?php if(is_array($focus)){foreach($focus as $mymps) { ?>                <div class="cell">
                    <a href="<?=$mymps['url']?>"><img src="<?=$mymps_global['SiteUrl']?><?=$mymps['image']?>"
                                                 alt="<?=$mymps['words']?>"></a>
                </div>
                <?php }} ?>
            </div>
            ...
            <ul class="list_normal list_news">
                <?php if(is_array($news)){foreach($news as $mymps) { ?>                <li class="img">
                    <a href="<?=$mymps['uri']?>" class="link">
                        <p class="img"><img src="<?=$mymps['imgpath']?>"
                                            onerror="this.src='<?=$mymps_global['SiteUrl']?>/images/nophoto.jpg'"/></p>
    ...
    <div class="index-category">
        <div class="index_slider">
            <div class="index_slider-wrap">
                <div class="page">
                    <?php $navigation = get_mobile_nav(2); ?>                    <a href="index.php?mod=category&catid=228" class="item food"
                       style=" width:50%;height: 100px;margin-top: 20px;">
                        <img src="/template/default/images/index/2.jpg" width="140">
                    </a>
                    <?php if(is_array($navigation)){foreach($navigation as $mymps) { ?>                    <a href="<?=$mymps['url']?>" class="item food"><? if($mymps['ico']) { ?>
                        <div class="icon"><img src="<?=$mymps_global['SiteUrl']?><?=$mymps['ico']?>"></div>
                        <?php } echo cutstr($mymps['title'],8); ?></a>
                    <?php }} ?>
                </div>
            </div>
        </div>
    ...
    <div id="tab_01" class="newsct">
        <div class="select_03 select_03_<?=$mymps_global['cfg_tpl_dir']?> tab-hd">
            <ul>
                ...
                <?php $ifnews = ifplugin('news'); ?>                <? if($ifnews) { ?>
                <li class="item current2"><a href="javascript:void(0);">热点资讯</a></li>
                <?php } else { ?>
                <li class="item current2"><a href="javascript:void(0);">最新发布</a></li>
                <?php } ?>
                <? if($mymps_global['cfg_if_corp'] == 1) { ?>
                <li class="item current3"><a href="javascript:void(0);">推荐商家</a></li>
                <?php } ?>
        ...
        <div>
            <ul class="list_normal first_bold tab-cont">
                ...
                <?php }} ?>
                <div class="inner_html"><a href="index.php?mod=category" class="comn-submit gray btn_block">进入分类信息频道</a>
                </div>
            </ul>
            <? if($ifnews) { ?>
            <ul class="list_normal first_bold tab-cont" style="display:none;">
                ...
                <?php }} ?>
                <div class="inner_html"><a href="index.php?mod=news" class="comn-submit gray btn_block">进入热点资讯频道</a>
                </div>
            </ul>
            <?php } else { ?>
            <ul class="list_normal first_bold tab-cont" style="display:none;">
            ...
            <?php $members = mymps_get_members(!$cityid ? 6 : NULL,NULL,NULL,NULL,2,NULL,NULL,$cityid); ?>            <ul class="list_normal first_bold tab-cont" style="display:none;">
                <?php if(is_array($members)){foreach($members as $k => $mymps) { ?>                <li><img src="<?=$mymps['prelogo']?>" class="img"> <a href="index.php?mod=store&id=<?=$mymps['id']?>"><?=$mymps['tname']?></a><span
                        class="jian"></span></li>
                <?php }} ?>
                <div class="inner_html"><a href="index.php?mod=corp" class="comn-submit gray btn_block">进入商家店铺频道</a>
                </div>
            </ul>
            <?php } ?>
        </div>
