                ...
                <center id="infobox">
                    <ul>
                        <div id="mr">
                            ...
                            <p>您发布的ID号为 <strong style="font-size: 18px;color: red;"><?=$ok['id']?></strong> &nbsp;的信息，目前为<b
                                    style=color:red>待审状态</b>，管理员审核通过后，就会在本网站上显示！<br/><br/><a
                                    href="<?=$mymps_global['SiteUrl']?>/<?=$mymps_global['cfg_postfile']?>">我要再发布一条信息&raquo;</a><br/><? if($r['userid']) { ?><a href="<?=$mymps_global['SiteUrl']?>/member/index.php?m=info"
                                              style="text-decoration:underline;">[我发布的所有信息]</a><?php } else { ?><a
                                        href="<?=$mymps_global['SiteUrl']?>"
                                        style="text-decoration:underline;">[点此返回网站首页]</a><?php } ?></p>
                            <?php } else { ?>
                            <h1 class="h">信息发布成功！</h1>
                            <p>您的信息 <a target="_blank" href="<?=$ok['info_uri']?>" class="mrtitle"><b><?=$ok['title']?></b></a>
                                &nbsp;已经成功发布</p>
                            <p><b><font color="#FF0000">温馨提示：</font></b> 该信息将在3分钟后显示在频道列表页上！ <? if($r['userid']) { ?><a
                                    href="<?=$mymps_global['SiteUrl']?>/member/index.php?m=info"
                                    style="text-decoration:underline;">[我发布的所有信息]</a><?php } else { ?><a
                                    href="<?=$mymps_global['SiteUrl']?>" style="text-decoration:underline;">[点此返回网站首页]</a><?php } ?>
                            </p>
                            <p style="padding:15px 0 0;">
                                <a class="button a xxl"
                                   href="javascript:setbg('置顶分类信息',538,248,'<?=$mymps_global['SiteUrl']?>/box.php?part=upgrade&id=<?=$ok['id']?>');"
                                   style="width:190px; margin-left:50px"><span><i></i>立即置顶该信息</span></a>
                                <a class="button c xxl"
                                   href="<?=$mymps_global['SiteUrl']?>/<?=$mymps_global['cfg_postfile']?>"><span><i></i>我要再发布一条信息&raquo;</span></a>
                            </p>
                            <?php } ?>
                        </div>
                    </ul>
