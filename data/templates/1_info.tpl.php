...
<head>
    ...
    <meta name="keywords"
          content="<? if($info['extra']) { ?><?php if(is_array($info['extra'])){foreach($info['extra'] as $mymps) { if($mymps['value']) { ?><?=$mymps['title']?><?=$mymps['value']?>,<?php } ?><?php }} ?><?php } ?><?=$info['title']?>"
    ...
    <link rel="stylesheet" href="<?=$mymps_global['SiteUrl']?>/template/default/css/customer/lightgallery.min.css">
                    ...
                    <div class="bd_lefter">
                        <div class="information_hd">
                            <ul>
                                ...
                                <div class="information_time">
                                    ...
                                    <span><font id="hit"><img src="<?=$mymps_global['SiteUrl']?>/images/loading.gif"
                                                              border="0" align="absmiddle"></font>次浏览</span>
                                    <span>
                                        <span class="STYLE1">信息编号：</span>
                                        <strong style="color: #0BB20C;font-size: 28px;"><?=$info['id']?></strong>
                                    </span>
                                    <span class="editor">
                                <li><a href="javascript:void(0);"
                                       onclick="setbg('置顶信息',538,248,'<?=$mymps_global['SiteUrl']?>/box.php?part=upgrade&id=<?=$info['id']?>');"
                                       style="color:red">置顶</a></li>
                                <li><a rel="nofollow" href="javascript:void(0);"
                                       onclick="setbg('收藏信息',538,270,'<?=$mymps_global['SiteUrl']?>/box.php?part=shoucang&infoid=<?=$info['id']?>')">收藏</a> &nbsp;| </li>
                                <li><a rel="nofollow" href="javascript:void(0);"
                                       onclick="setbg('删除信息',538,270,'<?=$mymps_global['SiteUrl']?>/box.php?part=delinfo&id=<?=$info['id']?>')"
                                       title="一旦删除将不能恢复！">删除</a> &nbsp;| </li>
                                <li><a rel="nofollow"
                                       href="<?=$mymps_global['SiteUrl']?>/<?=$mymps_global['cfg_postfile']?>?action=edit&id=<?=$info['id']?>"
                                       target="_blank">修改</a> &nbsp;| </li>
                                <li><a class="report" href="javascript:void(0);"
                                       onclick="setbg('举报信息',470,300,'<?=$mymps_global['SiteUrl']?>/box.php?part=report&id=<?=$info['id']?>&infotitle=<?=$info['title']?>');">举报</a> &nbsp;| </li>
                              </span>
                                </div>
                            </ul>
                        </div>
                        <div class="clear"></div>
                        <div class="extra_contact" style="position: relative">
                            <? if($info['status']) { ?>
                            <p class="status_img">
                                已售</p>
                            <?php } ?>
                            <div class="extra">
                                <div class="zoombox">
                                    <div class="zoompic"><img
                                            src="<?=$mymps_global['SiteUrl']?><? echo $info['img_path'] ? $info['img_path'] : $mymps_global['SiteUrl'].'/images/nophoto.jpg';; ?>"
                                            title="<?=$info['title']?>" alt="<?=$info['title']?>"/></div>
                                    <div class="clear"></div>
                                    <div class="clear"></div>
                                    <div class="sliderbox">
                                        ...
                                        <div class="sliderr" id="thumbnail">
                                            <ul>
                                                <?php $i =1; ?>                                                <?php if(is_array($info['image'])){foreach($info['image'] as $mymps) { ?>                                                <li <? if($i==1) { ?>class="currentt"<?php } ?>><a
                                                        href="<?=$mymps_global['SiteUrl']?><?=$mymps['path']?>"
                                                        target="_blank"><img
                                                        src="<?=$mymps_global['SiteUrl']?><?=$mymps['prepath']?>" width="43"
                                                        height="37" alt="<?=$info['title']?>" title="<?=$info['title']?>"/></a>
                                                </li>
                                                <?php $i++; ?>                                                <?php }} ?>
                                            </ul>
                                        </div>
                            ...
                            <div class="contact">
                                <ul>
                                    ...
                                    <li style="<? if(in_array($mymps['title'],array('价格','租金'))) { ?>color:red;<?php } ?>"><span><?=$mymps['title']?>：</span><? if(in_array($mymps['value'],array('0元','0元/月','0元/平米/天','0万元','0.00元','0.00万元'))) { ?>面议<?php } else { echo str_replace('.00','',$mymps['value']); ?><?php } ?>
                                    </li>
                                    <?php } ?>
                                    <?php }} ?>
                                    <? if($info['areaname']) { ?>
                                    <li><span>地址：</span><?=$info['areaname']?> <? if($info['ip'] == 'wap') { ?><font class="font"
                                                                                                       color="green">手机发布</font><?php } else { ?><font
                                            class="font"><?=$info['ip']?></font> <font class="font" color="green"><?=$info['ip2area']?></font><?php } ?>
                                    </li>
                                    <?php } ?>
                                    <div class="clear"></div>
                                    <ul class="contacter">
                                        ...
                                        <div class="qqbm"><span>&nbsp;</span><a href="javascript:void(0);"
                                                                                onclick="setbg('查看联系方式',550,320,'<?=$mymps_global['SiteUrl']?>/box.php?part=seecontact&infoid=<?=$info['id']?>&if_view=<?=$info['contactview']?>')"
                                                                                class="_bm">查看联系方式</a></div>
                                        <?php } elseif($info['usecoin'] < 0 && $info['contactview'] == 1) { ?>
                                        <div class="clear5"></div>
                                        <div class="notice">
                                            <ul>该信息联系方式已被隐藏，登录后方可查看!</ul>
                                        </div>
                                        <?php } else { ?>
                                        <? if($info['contactview'] == 1) { ?>
                                        <li><span>联系人：</span><font class="tel"><?=$info['contact_who']?></font></li>
                                        <li class="qqbm"><span>电话：</span><font class="tel red"><?=$info['telephone']?></font>
                                            <a rel="nofollow" href="javascript:void(0);"
                                               onclick="setbg('查看完整电话',420,520,'<?=$mymps_global['SiteUrl']?>/box.php?part=seecontact_tel&id=<?=$info['id']?>&tel_base64=<?=$info['tel_base64']?>')"
                                               class="_qq">点击查看完整号码</a></li>
                                        <div class="clear5"></div>
                                        <div class="notice">
                                            <ul>
                                                <strong><?=$mymps_global['SiteName']?>提醒您：</strong>让你提前汇款，或者价格明显低于市价，均有骗子嫌疑，不要轻易相信。
                                            </ul>
                                        </div>
                                        <?php } else { ?>
                                        <div class="clear5"></div>
                        ...
                        <div class="view_bd">
                            <div class="tablist">
                                ...
                                <div class="bd">
                                    <p id="image-box">
                                        <?php if(is_array($info['image'])){foreach($info['image'] as $mymps) { ?>                                        <img src="<?=$mymps_global['SiteUrl']?><?=$mymps['path']?>"
                                             class="imginfo" title="<?=$info['title']?>"
                                             alt="<?=$info['title']?>" data-src="<?=$mymps_global['SiteUrl']?><?=$mymps['path']?>">
                                        <?php }} ?>
                                    </p>
                                </div>
                                <div class="clear"></div>
                            ...
                            <div class="tablist">
                                <div class="bd">
                                    <p>
                                        <iframe src="<?=$mymps_global['SiteUrl']?>/map.php?title=<?=$info['title']?>&isshow=1&p=<?=$info['mappoint']?>&width=800&height=360"
                                                height="380" width="800" frameborder="0" scrolling="no"></iframe>
                                    </p>
                                </div>
                            </div>
                            <?php } ?>
                        ...
                        <div class="relates">
                            <ul>
                                ...
                                <li>
                                    <div class="relateimg"><a href="<?=$mymps['uri']?>" target="_blank"><img
                                            src="<? echo $mymps['img_path']? $mymps['img_path'] : $mymps_global['SiteUrl'].'/images/nophoto.jpg'; ?>"
                                            border="0"></a></div>
                                    <div class="relatetit">
                ...
                <div class="bd_right">
                    <div class="boxx">
                        <div class="con">
                            <ul class="school_logo" style="text-align:center; margin-bottom:5px;">
                                <img class="userlogo"
                                     src="<? echo $member['logo'] ? $member['logo'] : $mymps_global['SiteUrl'].'/images/nophoto.jpg'; ?>"/>
                                <? if($info['usecoin'] >= 0 || $info['contactview'] != 1) { ?>
                                <div class="clear"></div>
                                <div class="username"><?=$info['title']?></div>
                                <?php } ?>
                                <div class="clear"></div>
                                <? if($info['ismember'] ==1) { ?>
                                <div class="regtime">注册时间：<? echo GetTime($member['jointime'],'Y-m-d'); ?></div>
                                <?php } ?>
                                <? if($member['if_corp'] == 1) { ?>
                                <div class="clear"></div>
                                <div class="certify"><img style="border:none;"
                                                          src="<?=$mymps_global['SiteUrl']?>/template/default/images/user2.gif"
                                                          align="absmiddle" title="商家"/></div>
                                <?php } ?>
                            </ul>
                            <? if($info['ismember'] ==1) { ?>
                            <div class="clearfix"></div>
                            <ul class="jibie">
                                ...
                                <li>信用等级：<img align="absmiddle"
                                              src="<?=$mymps_global['SiteUrl']?>/images/credit/<?=$member['credits']?>.gif"
                                              alt="信用值:<?=$member['credit']?>"/></li>
                            </ul>
                            <div class="clear"></div>
                            <ul class="renzheng Identity">
                                <? if($member['per_certify'] ==1) { ?><p><img src="<?=$mymps_global['SiteUrl']?>/images/person1.gif"
                                                                     alt="已通过身份证认证"/> 已通过身份证认证 </p> <?php } else { ?><p><img
                                    src="<?=$mymps_global['SiteUrl']?>/images/person0.gif" alt="未上传身份证认证"/> 未上传身份证认证 </p>
                                <?php } ?>
                                <? if($member['com_certify'] ==1) { ?><p><img src="<?=$mymps_global['SiteUrl']?>/images/company1.gif"
                                                                     alt="已通过营业执照认证"/> 已通过营业执照认证 </p> <?php } else { ?><p><img
                                    src="<?=$mymps_global['SiteUrl']?>/images/company0.gif" alt="未上传营业执照认证"/> 未上传营业执照认证 </p>
                                <?php } ?>
                            </ul>
                            <?php } ?>
                        </div>
                    ...
                    <div class="boxer cfix">
                        ...
                        <div class="boxer_bd cfix short">
                            <ul>
                                <?php $relate_cat = get_categories_tree($info['parentid']); ?>                                <?php if(is_array($relate_cat)){foreach($relate_cat as $mymps) { ?>                                <?php if(is_array($mymps['children'])){foreach($mymps['children'] as $w) { ?>                                <li><a target="_blank" href="<?=$w['uri']?>" title="<?=$city['cityname']?><?=$w['catname']?>"><?=$w['catname']?></a>
                                </li>
                                <?php }} ?>
                                <?php }} ?>
                            </ul>
                    ...
                    <div class="boxer">
                        <span style="color: red;font-size: 24px;display: block;line-height: 44px;">
                            本站郑重承诺，诚信经营、不赚差价、交易透明、买卖双方面对面议价不收取任何中间费用。
                        </span>
                    </div>
                </div>
            </div>
...
<div class="footer_new">
    ...
    <div class="foot_powered">
    	Copyright &copy; <?=$mymps_global['SiteName']?>版权所有 <a href="http://www.miibeian.gov.cn"><?=$mymps_global['SiteBeian']?></a><font id="foots">Powered by<a href="#"><?=$mymps_global['SiteName']?></a></font><?=$mymps_global['SiteStat']?> <? if($cachetime) { ?>This page is cached at <? echo GetTime($timestamp,'Y-m-d H:i:s'); ?><?php } else { ?><font class="none_<?=$mymps_mymps['debuginfo']?>"><?php $mtime = explode(' ', microtime());$totaltime = number_format(($mtime['1'] + $mtime['0']-$mymps_starttime), 6); echo ' , Processed in '.$totaltime.' second(s) , '.$db->query_num.' queries'; ?></font><?php } ?>
    </div>
</div>
...
<script type="text/javascript">
    var bdsize = '16';
    var infoid = '<?=$info['id']?>';
    loadDefault(['slider', 'baidushare', 'info']);
