...
$mymps_admin_info_type = array(
    "text" => '
		...
		',
    "timer" => '<input type="hidden" size="50" name="rules[timer][maxlength]" value="" >',
    "rangeTimer" => '<input type="hidden" size="50" name="rules[rangeTimer][maxlength]" value="" >',
    "textarea" => '
		<tr>
		<td bgcolor="#f5fbff" width="45%" >
		<b>字符最大长度:</b>
...
function get_info_var_type($type, $name = "", $rules = "", $value = "", $forward = 'back', $title = '', $require = '')
{
    ...
    if ($forward == 'back') {
        switch ($type) {
            ...
            case 'timer':
                $mymps_rule_str = "";
                $mymps .= "<input id=\"" . $name . "\"  name=\"extra[" . $name . "]\" value=\"" . $value . "\"  class=\"datepicker input input-large\" type=\"text\"  " . $required . "> " . $mymps_rule_str;
                break;
            case 'rangeTimer':
                $mymps_rule_str = "";
                $mymps .= "<input id=\"" . $name . "\" name=\"extra[" . $name . "]\" value=\"" . $value . "\"  class=\"rangeTimer input input-large\" type=\"text\"  " . $required . "> " . $mymps_rule_str;
                break;
            case 'textarea':
        ...
        switch ($type) {
            ...
            case 'timer':
                $mymps_rule_str = "";
                $mymps .= "<input  name=\"extra[" . $name . "]\" value=\"" . $value . "\"  class=\"datepicker input input-large\" type=\"text\"  " . $required . "> " . $mymps_rule_str;
                break;
            case 'rangeTimer':
                $mymps_rule_str = "";
                $mymps .= "<input  name=\"extra[" . $name . "]\" value=\"" . $value . "\"  class=\"rangeTimer input input-large\" type=\"text\"  " . $required . "> " . $mymps_rule_str;
                break;
            case 'textarea':
                $mymps_rule_str = $rules[maxlength] ? "<br />不得超过" . $rules[maxlength] . "个字符" : "";
...
function return_category_info_options($modid = '', $edit_id = '')
{
    ...
    if ($modid > 1) {
        ...
        if ($row[options]) {
            $resquery = $db->query("SELECT optionid,title,identifier,type,rules,required FROM `{$db_mymps}info_typeoptions` WHERE optionid IN ($row[options]) ORDER BY displayorder DESC ");
