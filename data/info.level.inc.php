...
$information_status = array();
$information_status[0] = '<font color=red>待售</font>';
$information_status[1] = '<font color=#006acd>已售</font>';

function GetInfoStatus($status = '', $formname = 'status')
{
    global $information_status;
    $mymps .= "<select name='$formname' id='$formname'>";
    $mymps .= "<option value = \"\">请选择信息状态</option>";
    foreach ($information_status as $k => $v) {
        if ($k == $status) $mymps .= "<option value='$k' selected style='background-color:#6EB00C;color:white'>$v</option>\r\n";
        else $mymps .= "<option value='$k'>$v</option>\r\n";
    }
    $mymps .= "</select>\r\n";
    return $mymps;
