...
$(document).ready(function(){
	console.log('sss')
		...
		draw.onclick=function(){
			...
			function start(e){
                ...
                context.beginPath();
				context.moveTo(e.clientX-canvas.offsetLeft,e.clientY-canvas.offsetTop-hh+361);
        ...
        wipe.onclick=function(){
			...
			function move(e){
				var e=e.changedTouches[0]
                context.clearRect(e.clientX-canvas.offsetLeft,e.clientY-canvas.offsetTop-hh+361,20,20);
			}
	...
	//生成图片
			...
			var flag = $(this).parent().children().attr('id');
			for(let i=0;i<savebtns.length;i++){
				...
				$.post("/upload_img.php?action=upload",{imgSrc},function(res){
					if(res.status){
						var html = '<input type="hidden" name="img[]" id="'+flag+'" value="'+res.path+'">';
						$("#form1").prepend(html);
						// alert(res.info+res.path);
					}else{
						alert(res.info);
					}
				},'json');
	...
	$('#fileInput').change(function(){
		...
		//生成图片
		$('.imageDiv .save').click(function(){
			var flag = $(this).parent().children().attr('id');
			var imgSrc = (canvas.toDataURL()); //获取图片的DataURL
			$(this).parent().children().eq(0).attr('src',URL.createObjectURL(dataURItoBlob(imgSrc)));
			var _this = $(this);
			context.clearRect(0,0,canvas.width,canvas.height);
			$('#canvas_graffiti').css('display','none');
			$.post("/upload_img.php?action=upload",{imgSrc},function(res){
