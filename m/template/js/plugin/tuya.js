...
$(document).ready(function(){
			var canvas_graffiti=document.getElementById('canvas_graffiti');
			canvas_graffiti.addEventListener("touchmove", function (e){
					e.preventDefault();
			})
		...
		var draw=document.getElementById('btn_draw');
        var wipe=document.getElementById('btn_wipe');
		...
		draw.onclick=function(){
			...
			function start(e){
                ...
                context.beginPath();
				context.moveTo(e.clientX-canvas.offsetLeft,e.clientY-canvas.offsetTop-45);
				document.addEventListener("touchmove",move)
			}
			function move(e){
				var e=e.changedTouches[0];
               // context.lineTo(e.clientX-canvas.offsetLeft,e.clientY-canvas.offsetTop-hh+361);
                  context.lineTo(e.clientX-canvas.offsetLeft,e.clientY-canvas.offsetTop-45);
                context.lineWidth=8;
                context.strokeStyle="red";
                context.stroke();
			}
			document.addEventListener("touchend", function(){
				canvas2.removeEventListener("touchstart",start);
        ...
        wipe.onclick=function(){
			...
			function move(e){
				var e=e.changedTouches[0]
                context.clearRect(e.clientX-canvas.offsetLeft,e.clientY-canvas.offsetTop-45,20,20);
			}
	...
	//清除
		...
		$("#close_window").bind("tap",function(){
			$("#forbiden_back").fadeOut(300);
			$("#pic_url").val('');
		});

		$("#open_pic").bind("tap",function(){
			$("#forbiden_back").fadeOut(300);
	...
	$('#fileInput').change(function(){
		var urlll=""
		$(".picDiv .imageDiv .drawimg").click(function(){
		...
		//保存图片
		$('#cavas_btn #save').click(function(){
			var imgSrc = (canvas.toDataURL()); //获取图片的DataURL
			var imgss=$('.imageDiv img');
			for(let i=0;i<imgss.length;i++){
				var imgid=imgss.eq(i).attr('src');
				if(urlll == imgid){
					var imgAttr = imgss.eq(i).attr('id');
					imgss.eq(i).attr('src',URL.createObjectURL(dataURItoBlob(imgSrc)));
					$('#canvas_graffiti').css('display','none');
					$.post("/upload_img.php?action=upload",{imgSrc},function(res){
						if(res.status){
							var html = '<input type="hidden" name="img[]" value="'+res.path+'">';
							html += '<input type="hidden" name="draw[]" value="'+imgAttr+'">';
							imgss.eq(i).before(html);
						}else{
							alert(res.info);
						}
					},'json');
		...
		//取消
		$('#cavas_btn #quxiao').click(function(){
			$('#canvas_graffiti').css('display','none');
		})
	...
	function open_img(url){
		...
		$(img).bind("load",function(){
			if($(this).attr('src') !='./images/post/defaultimg.gif'){
				$('#canvas_graffiti').css('display','block');
