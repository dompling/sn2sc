...
$(document).ready(function(){
		...
		var top1=$('#canvas_graffiti').offset().top;
		var  left1=$('#canvas_graffiti').offset().left;
		//var top2=$(".publish-detail").offset().top;
		$(img).bind("load",function(){
			...
			$(canvas_color).bind("mousedown",function(event){
				console.log(top1,event.pageY,canvas_color.offsetTop)
				var idx = ((event.clientX-canvas_color.offsetLeft-left1-1) + (event.pageY-1-canvas_color.offsetTop-top1-1) * canvas_color_data.width) * 4;
			...
			$(canvas_color).bind("mousemove",function(event){
				if(color_changeable==false)
					return;
				var x=event.clientX-canvas_color.offsetLeft-1-left1;
				if(x>=canvas_color_data.width||x<0)
					return;
				var y=event.pageY-canvas_color.offsetTop-1-top1;
